...
class AbstractSkipThoughts(nn.Module):
    ...
    def _load_emb_params(self):
        ...
        params = numpy.load(path_params, encoding='latin1', allow_pickle=True) # added allow_pickle=True to load from python2
        return params
    ...
    def _process_lengths(self, input):
        ...
        batch_size = input.size(0)
        if batch_size == 1:
            lengths = [max_length - input.data.eq(0).sum(1).squeeze()] # removed list
        else:
